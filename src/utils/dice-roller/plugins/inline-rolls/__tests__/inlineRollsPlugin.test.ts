...
import { parseInlineRollAst, extractInlineRolls, evaluateInlineRollNode, assignInlineRollIndices, inlineRollsPlugin } from '../index';
...
describe('Inline Rolls Plugin > Parsing', () => {
  ...
  it('should throw on invalid inline roll syntax', () => {
    expect(() => parseInlineRollAst('[1d20+5]')).toThrow('Invalid inline roll syntax');
    expect(() => parseInlineRollAst('[[1d20+5]')).toThrow('Invalid inline roll syntax');
  });
...
describe('Inline Rolls Plugin > Evaluation', () => {
  it('should evaluate an inline roll node', () => {
    ...
    (evaluate as Mock).mockReturnValue({ total: 5, rolls: [], warnings: [], details: {} });

    const result = evaluateInlineRollNode(node, context);

    expect(parseInput).toHaveBeenCalledWith('5');
    expect(evaluate).toHaveBeenCalledWith({ type: 'number', value: 5 }, context);
  ...
  it('should throw for non-inline-roll node', () => {
    ...
    expect(() => evaluateInlineRollNode(node, context)).toThrow('Expected inline roll node');
  });

  it('should throw for missing index', () => {
    const node = { type: 'inline-roll', expression: '1d20+5' };
    const context = {} as any;
    expect(() => evaluateInlineRollNode(node, context)).toThrow('Inline roll node missing index');
  });
});
