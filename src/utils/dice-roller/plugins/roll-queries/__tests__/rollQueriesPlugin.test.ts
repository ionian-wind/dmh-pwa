...
describe('Roll Queries Plugin > Parsing', () => {
  ...
  it('should throw on invalid roll query syntax', () => {
    expect(() => parseRollQueryAst('{Bonus|0}')).toThrow(
  ...
  it('should throw on empty roll query', () => {
    expect(() => parseRollQueryAst('?{}')).toThrow(
      'Roll query must have at least a prompt',
    );
  });
});
...
describe('Roll Queries Plugin > Evaluation', () => {
  ...
  it('should throw error for missing user input', () => {
    ...
    expect(() => evaluateRollQueryNode(node, context)).toThrow(
      'User input not provided for query: Bonus',
    );
  });
  ...
  it('should throw error for invalid option', () => {
    ...
    expect(() => evaluateRollQueryNode(node, context)).toThrow(
      "Invalid option 'Bow' for query 'Weapon'. Valid options: Sword, Axe",
    );
  });
  ...
  it('should throw error if nesting limit is exceeded', () => {
    ...
    expect(() => evaluateRollQueryNode(node, context)).toThrow(
      'Roll query recursion limit exceeded (99 levels)',
    );
  });
