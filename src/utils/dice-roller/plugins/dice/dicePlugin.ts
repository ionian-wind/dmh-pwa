...
export function parseDiceAst(input: string): DiceAstNode {
  ...
  try {
    ...
    let tokensArr: any[] = [];
    if (
      Array.isArray(lexResult.tokens) &&
      lexResult.tokens.length > 0 &&
      typeof lexResult.tokens[0] === 'object' &&
      'image' in lexResult.tokens[0] &&
      'tokenType' in lexResult.tokens[0]
    ) {
      tokensArr = lexResult.tokens;
    }
    const debugTokens = tokensArr.map((t) => ({
      image: t.image,
...
function applyModifiers(
  ...
  if (eIndex !== -1) {
    ...
    localModifiers = localModifiers
      .slice(0, eIndex)
      .concat(localModifiers.slice(eIndex + 1));
  }
  ...
  if (eModifier) {
    const lastIdx = localModifiers.findIndex((m) =>
      ['success', 'eq', 'gt', 'lt', 'cs', 'cf', 's', 'f'].includes(m.type),
    );
    if (lastIdx !== -1) {
  ...
  for (let i = 0; i < localModifiers.length; i++) {
    ...
    switch (modifier.type) {
      case 'kh': // Keep highest
        result = keepHighest(
          result,
          validatePositiveInteger(modifier.value, 'keep highest count'),
        );
        break;
      ...
      case 'kl': // Keep lowest
        result = keepLowest(
          result,
          validatePositiveInteger(modifier.value, 'keep lowest count'),
        );
        break;
      ...
      case 'dh': // Drop highest
        result = dropHighest(
          result,
          validatePositiveInteger(modifier.value, 'drop highest count'),
        );
      ...
      case 'dl': // Drop lowest
        result = dropLowest(
          result,
          validatePositiveInteger(modifier.value, 'drop lowest count'),
        );
        // Only keep the remaining dice after dropping
        break;

      case 'k>': // Keep dice above threshold
        const keepAboveThreshold = validatePositiveInteger(
          modifier.value,
      ...
      case 'k<': // Keep dice below threshold
        const keepBelowThreshold = validatePositiveInteger(
          modifier.value,
      ...
      case 'r': // Reroll
        const rerollTarget = validatePositiveInteger(
          modifier.value,
          'reroll target',
        );
        ...
        result = result.map((roll) => {
          ...
          while (attempts < maxAttempts) {
            const shouldReroll = (() => {
              switch (operator) {
                case '>':
                  return currentRoll > rerollTarget;
                case '<':
                  return currentRoll < rerollTarget;
                case '>=':
                  return currentRoll >= rerollTarget;
      ...
      case 'ro': // Reroll once
        const roTarget = validatePositiveInteger(
          modifier.value,
          'reroll once target',
        );
      ...
      case 'ra': // Reroll until success
        const raTarget = validatePositiveInteger(
          modifier.value,
          'reroll until success target',
        );
      ...
      case 'explode': // Explosion
        ...
        if (modifier.value === 'recursive') {
          ...
          if (
            nextModifier &&
            (nextModifier.type === 'lt' ||
              nextModifier.type === 'gt' ||
              nextModifier.type === 'eq')
          ) {
            explosionOperator =
              nextModifier.type === 'lt'
                ? '<'
              ...
              : modifier.type === 'lt'
                ...
                : modifier.type === 'cs'
                  ...
                  : modifier.type === 'cf'
                    ...
                    : '=');
        const target =
          typeof modifier.target !== 'undefined'
            ? modifier.target
            : typeof modifier.value !== 'undefined'
              ? parseInt(modifier.value, 10)
              : undefined;
      ...
      case 'mi': // Minimum
        const minValue = validatePositiveInteger(
          modifier.value,
          'minimum value',
        );
      ...
      case 'ma': // Maximum
        const maxValue = validatePositiveInteger(
          modifier.value,
          'maximum value',
        );
      ...
      case 'm': // Matching
        ...
        const matches = Array.from(counts.entries()).filter(
  ...
  if (eModifier) {
    if (!lastSuccessModifier) {
      warnings.push(
        'Exhaustive (e) operator requires a preceding success/eq/gt/lt/cs/cf modifier',
      );
      details.exhaustive = { cycles: 0, totalSuccesses: 0 };
      result = [0];
    } else {
      const exOperator =
        lastSuccessModifier.operator ||
            ...
            : lastSuccessModifier.type === 'lt'
              ...
              : lastSuccessModifier.type === 'cs'
                ...
                : lastSuccessModifier.type === 'cf'
                  ...
                  : '=');
      const exTarget =
        typeof lastSuccessModifier.target !== 'undefined'
          ? lastSuccessModifier.target
          : typeof lastSuccessModifier.value !== 'undefined'
            ? parseInt(lastSuccessModifier.value, 10)
            : undefined;
      if (typeof exTarget === 'undefined') {
        ...
        while (cycle < maxCycles) {
          ...
          if (typeof window === 'undefined') {
            console.log(`[Exhaustive] Cycle ${cycle + 1}:`, {
              currentRolls,
              successes,
              totalSuccesses,
            });
...
// Evaluate dice nodes
export function evaluateDiceNode(
  node: DiceAstNode,
  context: DiceEvaluationContext,
): DiceEvaluationResult {
  debugLog(
    'evaluateDiceNode called with:',
    JSON.stringify(node),
  ...
  if (node.type === 'function') {
    ...
    const args = node.args.map(
      (arg) => evaluate(arg as DiceAstNode, context).total,
    );
  ...
  if (node.type === 'macro') {
    ...
    if (typeof window === 'undefined') {
      console.log(
        '[dicePlugin] evaluateDiceNode: delegating macro node to core.evaluate',
        JSON.stringify(node),
      );
    }
  ...
  if (node.type === 'group') {
    ...
    const results = node.expressions.map((expr) =>
      evaluateDiceNode(expr, context),
    );
    return {
  ...
  return {
    total: 0,
    rolls: [],
    warnings: [`Unhandled node type: ${node.type}`],
    details: {},
  };
}
