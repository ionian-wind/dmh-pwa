...
describe('dice plugin', () => {
  ...
  describe('extract', () => {
    it('extracts all Dice and Group nodes from AST', () => {
      const ast = {
        type: 'Group',
        items: [
          { type: 'Dice', count: 2, sides: 6, fudge: null, custom: null, plugin: 'dice' },
          { type: 'Dice', count: 1, sides: 8, fudge: null, custom: null, plugin: 'dice' },
        ],
        plugin: 'dice',
      };
      const result = extractDice(ast);
      expect(result.length).toBe(3);
      expect(result.filter(n => n.type === 'Dice').length).toBe(2);
      expect(result.filter(n => n.type === 'Group').length).toBe(1);
    });
  });
}); 