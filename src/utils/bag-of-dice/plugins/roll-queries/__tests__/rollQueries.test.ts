...
describe('Roll Queries Plugin', () => {
  ...
  describe('extract', () => {
    ...
    it('extracts queries from AST', () => {
      const ast = parseAndAst('?{A|1|2}');
      const extracted = rollQueriesPlugin.extract!(ast);
      expect(extracted.queries.length).toBe(1);
      expect(extracted.queries[0].prompt).toBe('A');
      expect(typeof extracted.queries[0].options[0].id).toBe('string');
      expect(typeof extracted.queries[0].options[1].id).toBe('string');
    });
  });
