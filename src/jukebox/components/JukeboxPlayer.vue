...
const progress = computed(() => {
  const value = playerStore.currentTime / playerStore.duration;
  console.log(value);
  return value;
});
...
<template>
  <div class="jukebox-player" :class="{ 'with-animated-bg': showAnimatedBg }" :style="gradientStyle">
    ...
    <QToolbar class="progress-bar q-pa-none q-gutter-x-sm" style="min-height:0;">
      <QBtnGroup flat class="playback-controls">
        <QBtn flat @click="playerStore.playPrev()" :disabled="!playerStore.hasPrevTrack" class="prev-track">
          <IconPlayerSkipBack />
        </QBtn>
        <QBtn flat @click="playerStore.togglePlay()" :disabled="!playerStore.canPlay" class="play-pause">
          <IconPlayerPause v-if="playerStore.isPlaying" />
          <IconPlayerPlay v-else />
        </QBtn>
        <QBtn flat @click="playerStore.playNext()" :disabled="!playerStore.currentTrack || !playerStore.hasNextTrack" class="next-track">
          <IconPlayerSkipForward />
        </QBtn>
      </QBtnGroup>
      ...
      <QBtnGroup flat class="playback-options">
        ...
        <QBtn
          flat
          :class="{ disabled: playerStore.repeatMode !== 'list' && playerStore.repeatMode !== 'track' }"
      ...
      <QBtnGroup flat>
        <QBtn flat @click="playerStore.toggleMute()" class="volume-button"
        ...
        >
          ...
          <QPopupProxy
            ...
            @mouseenter="isVolumePopupOpen = true"
          >
            <div class="volume-popup q-pa-sm" style="width: 120px;" @wheel.prevent="handleVolumeWheel">
              <QLinearProgress
                :instant-feedback="true"
              ...
              />
              <div class="text-center text-caption q-mt-xs">{{ Math.round(playerStore.volume * 100) }}%</div>
            </div>
          </QPopupProxy>
        </QBtn>
      </QBtnGroup>
...
.progress-bar-time.time-current, .progress-bar-seek, .playback-options .q-btn.disabled {
