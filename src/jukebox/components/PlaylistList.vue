...
<template>
  <div class="q-pa-none q-gutter-none q-mb-none playlist-list-container column full-height">
    ...
    <div>
      <QList class="q-pa-none q-mb-none">
        <QItem>
          ...
          <QItemSection side>
            <QBtn flat @click="openPlaylistModal(null)" variant="light"><IconPlus /></QBtn>
          </QItemSection>
        </QItem>
        <QSeparator />
    ...
    <QScrollArea class="q-pa-none q-mb-none col-grow" style="min-height:0;">
      ...
      >
        <template #item="{ element: playlist }">
          <QItem clickable :active="selectedPlaylistId === playlist.id" v-ripple @click="setSelectedPlaylist(playlist.id)" class="playlist-item">
            <QItemSection>
              {{ playlist.name }}
            </QItemSection>
            <QItemSection side>
              <QBtnGroup flat>
                <QBtn flat @click.stop="openPlaylistModal(playlist)"><IconPencil /></QBtn>
                <QBtn flat @click.stop="removePlaylist(playlist.id)"><IconX /></QBtn>
              </QBtnGroup>
            </QItemSection>
          </QItem>
        </template>
