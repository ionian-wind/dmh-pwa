...
function handleWheel(event: WheelEvent) {
  const newVolume = Math.min(
    1,
    Math.max(0, playerStore.volume - event.deltaY * 0.001),
  );
...
<template>
  <div
    ...
    :class="{ 'with-animated-bg': showAnimatedBg }"
    :style="gradientStyle"
  >
    <div class="jukebox-player-content">
      <div class="track-info-row">
        ...
        <div v-if="showArtwork" class="track-artwork-container">
          <div
            v-if="playerStore.currentTrack?.picture"
            :style="pictureStyle"
            class="track-artwork"
          ></div>
        ...
        <div class="track-info-top">
          <div class="title">
            {{ playerStore.currentTrack?.title || 'No track selected' }}
          </div>
          <div class="artist">
      ...
      <div class="player-controls-bottom">
        ...
        <div class="progress-bar">
          <span
            class="progress-bar-time time-current"
          ...
          >
            ...
            <template v-else>
              -{{
                formatTime(
                  (playerStore.duration || 0) - playerStore.currentTime,
                )
              }}
            </template>
          </span>
          ...
          />
          <span class="progress-bar-time time-duration">{{
...
.playback-options button:hover:not(:disabled) {
  background: none;
  transform: scale(1.1);
}
.controls button:active:not(:disabled),
.volume-button:active:not(:disabled),
.playback-options button:active:not(:disabled) {
  transform: scale(0.95);
}
.controls button:disabled,
.volume-button:disabled,
.playback-options button:disabled {
  color: var(--jp-color-primary-light);
  cursor: not-allowed;
  opacity: 0.6;
  transform: none;
...
.playback-options button:disabled:hover {
  transform: none;
}
