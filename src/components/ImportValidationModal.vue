<template>
  ...
  >
    <div v-if="result.treeMatchesModule" class="text-success mb-3">
      <IconCircleCheck class="icon align-middle me-1" /> Tree structure in
      module.json matches tree.json
    </div>
    <div v-else class="text-danger mb-3">
      <IconX class="icon align-middle me-1" /> Tree structure in module.json
      does not match tree.json
    </div>
    ...
    <div class="mb-3">
      ...
      <ul v-if="result.missingNoteIds.length > 0">
        ...
        <li v-if="result.missingNoteIds.length > 10">
          ...and {{ result.missingNoteIds.length - 10 }} more
        </li>
      </ul>
      <div v-else class="text-success">
        <IconCircleCheck class="icon align-middle me-1" /> All noteIds in tree
        have corresponding note files
      </div>
    </div>
    ...
    <div>
      ...
      <ul v-if="result.orphanedNoteFiles.length > 0">
        ...
        <li v-if="result.orphanedNoteFiles.length > 10">
          ...and {{ result.orphanedNoteFiles.length - 10 }} more
        </li>
      </ul>
      <div v-else class="text-success">
        <IconCircleCheck class="icon align-middle me-1" /> All note files are
