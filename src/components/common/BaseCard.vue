...
<template>
  <QCard>
    <QCardSection v-if="$slots.header || title" class="base-card-header" :class="{ 'no-border': noBorderAfterHeader }" @click="$emit('view')" style="cursor: pointer">
      <slot name="header">
    ...
    <QCardActions v-if="$slots.actions || showView || showEdit || showDelete" class="base-card-actions" align="between">
