...
export function useModalState() {
  ...
  const openMentionModal = (modalId: string, entity: any, kind: string) => {
    ...
    if (prev && prev.kind === kind && prevId && entityId && prevId === entityId) {
  ...
  const closeMentionModal = (modalId: string) => {
    ...
    if (mentionModalStack.value.length && mentionModalStack.value[mentionModalStack.value.length - 1].modalId === modalId) {
      mentionModalStack.value.pop();
    } else {
      // Remove all occurrences (shouldn't happen in normal flow)
      mentionModalStack.value = mentionModalStack.value.filter(m => m.modalId !== modalId);
    }
    closeModal(modalId);
  };

  const currentMentionModal = computed(() => mentionModalStack.value.length > 0 ? mentionModalStack.value[mentionModalStack.value.length - 1] : null);
