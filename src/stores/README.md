...
### 2. Store Definition
```typescript
export const useEntityStore = defineStore('entities', (): StandardizedStore<EntityType> => {
  ...
  const create = async (item: Omit<EntityType, 'id' | 'createdAt' | 'updatedAt'>) => {
    const newItem = await base.create(item);
    // Additional logic (e.g., mentions, indexing)
    return newItem;
  };

  const update = async (id: string, item: Partial<Omit<EntityType, 'id' | 'createdAt' | 'updatedAt'>>) => {
...
const load = async () => {
  ...
  try {
    const migratedData = await migrateStorageData('storage-key', migrateFunction, []);
