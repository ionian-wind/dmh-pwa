...
const notes = computed(() =>
  noteStore.items.filter(
    (note) => note.moduleId === props.moduleId && note.hidden === true,
  ),
);

// Convert ModuleTreeNode[] to ExtendedTreeNode[] for the Tree component
function convertToExtendedTree(
  ...
  return nodes.map((node) => {
    ...
    return {
      ...
      children: node.children
        ? convertToExtendedTree(node.children, notes)
        : undefined,
    };
  });
}
...
const localTree = ref<ExtendedTreeNode[]>(
  convertToExtendedTree(deepUnwrap(props.noteTree), deepUnwrap(notes.value)),
);
...
watch(
  ...
  () => {
    localTree.value = convertToExtendedTree(
      deepUnwrap(props.noteTree),
      deepUnwrap(notes.value),
    );
...
async function handleRemove(node: ExtendedTreeNode) {
  ...
  async function removeNotesRecursive(n: ExtendedTreeNode) {
    ...
    // Remove children
    if (n.children)
      for (const child of n.children) await removeNotesRecursive(child);
  }
